...
def module_list(request):
	...
	if request.user.is_authenticated:
		completed = TrainingProgress.objects.filter(user=request.user).values_list('module_id', flat=True)
	return render(request, 'training_list.html', {'modules': modules, 'completed': completed})
